...
gmm_model = GMM(n_components = 5, covariance_type = 'tied').fit(pixels_reshape)
...
img_y = cv2.imread("../data/Parking Lot/Lot3.tif")
...
for index in range(num_files):
    ...
    output_name = "Output/Gaussian_MM/Transformed_"+filename+".jpg"
    ...
    output_name = "Output/Gaussian_MM/Masked_"+filename+".jpg"
    os.makedirs(os.path.dirname(output_name), exist_ok=True)
    plt.savefig(output_name)
    plt.clf()
    ...
    output_name = "Output/Gaussian_MM/line_detected_"+filename+".jpg"
    os.makedirs(os.path.dirname(output_name), exist_ok=True)
    plt.savefig(output_name)
    plt.clf()
